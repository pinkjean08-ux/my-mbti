...
POKEMON_DATA = {
    ...
    "ISFJ": {
        ...
        "description": (
            ...
            "다친 포켓몬을 돌보는 해피너스처럼 따뜻한 배려로 "
            "주변 사람에게 안정감과 위로를 줍니다."
        ),
        "keywords": "배려 · 헌신 · 따뜻함",
    },
    "INFJ": {
    ...
    "INFP": {
        ...
        "description": (
            ...
            "겉으로 보이는 것보다 훨씬 다양한 가능성을 품고 있습니다."
        ),
        "keywords": "상상력 · 가치 · 가능성",
    },
    "INTP": {
        ...
        "description": (
            ...
            "뛰어난 두뇌를 가진 후딘처럼 복잡한 문제를 논리적으로 분석하며 "
            "새로운 가능성을 생각합니다."
        ),
        "keywords": "논리 · 탐구 · 아이디어",
    },
    ...
    "ENTJ": {
        ...
        "description": (
            ...
            "강력한 힘을 가진 망나뇽처럼 도전적인 상황에서도 "
            "자신감을 가지고 목표를 향해 나아갑니다."
        ),
        "keywords": "목표 · 전략 · 추진력",
    },
}
...
# ---------------------------------------------------------
# 간단한 화면 디자인
# CSS는 화면 장식에만 사용하고 결과 내용은 HTML로 출력하지 않음
# ---------------------------------------------------------
st.markdown(
    """
    <style>
    .stApp {
        background: linear-gradient(135deg, #fffaf0 0%, #f3efff 100%);
    }
...
# 제목
# ---------------------------------------------------------
st.title("🔮 MBTI 포켓몬 연구소")
st.write("MBTI를 선택하면 성격 특성과 어울리는 포켓몬을 추천해 드립니다.")
...
# MBTI 선택
# ---------------------------------------------------------
mbti_options = ["MBTI를 선택하세요"] + list(POKEMON_DATA.keys())
...
if recommend_button:
    if selected_mbti == "MBTI를 선택하세요":
        ...
        st.subheader(f"{selected_mbti}에게 어울리는 포켓몬")
        ...
        # 포켓몬 이름과 정보
        st.header(result["pokemon"])
        st.caption(f"포켓몬 타입: {result['type']}")
        ...
        st.success(
            f"{selected_mbti}의 성향과 {result['pokemon']}의 특징이 잘 어울립니다."
        )
...
st.caption(
    "이 결과는 재미를 위한 추천이며 공식 MBTI 검사나 심리학적 진단이 아닙니다."
)
